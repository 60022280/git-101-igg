...
export default function MovieDetail({ navigation, route}) {
    ...
    useEffect(() => {
        ...
        axios.get(`https://movie-api.igeargeek.com/movies/${itemId}`).then((res) => {
                ...
                <View style= {{flex: 2, padding: 15, backgroundColor: '#362420'}}>
                    <View style= {{flexDirection: 'row'}}>
                        ...
                        <View style={{flex:2, marginLeft: 15}}>
                            ...
                            <Text style={{ color: "white" }}>ประเภท:{ movies.genre ? movies.genre.join(', ') : ''}</Text>
                            <Text style={{ color: "white"}}>วันที่เข้าฉาย :{movies.showingAt}</Text>
                            <Text style={{ color: "white"}}>ระยะะเวลา : {movies.duration} นาที</Text>
